...
import DocumentsUpload, { DocumentTag } from "@/components/DocumentsUpload";
...
import { FormRegisterSellerProps, RegisterSellerValues } from "./interface";

// Documentos exigidos no KYC de vendedor (verified_seller). O título vem do i18n.
const REQUIRED_DOCS: { id: string; tag: DocumentTag }[] = [
  { id: "socialContract", tag: "basic" },
  { id: "legalRep", tag: "basic" },
  { id: "addressProof", tag: "verified" },
  { id: "anatelLicense", tag: "verified" },
  { id: "bankData", tag: "verified" },
  { id: "fiscalDeclaration", tag: "complete" },
];

const STEPS: { titleKey: string; fields: (keyof RegisterSellerValues)[] }[] = [
  {
  ...
  {
    titleKey: "Register.steps.documents",
    fields: ["documents"],
  },
];
      ...
      <Stack p={"32px"} bg={"#151B2D"} rounded={"16px"}>
            ...
            render={({ field }) => (
              <DocumentsUpload
                documents={REQUIRED_DOCS.map((d) => ({
                  id: d.id,
                  title: t(`Register.docs.items.${d.id}`),
                  tag: d.tag,
                }))}
        ...
        <Flex gap={3} mt={"32px"}>
          ...
          {step < lastStep ? (
            ...
            // Documentos são opcionais no cadastro: pode pular e verificar o e-mail.
            <Button
              flex="1"
              type="submit"
